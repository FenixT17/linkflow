...
function ResetPasswordForm() {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    setLoading(true);
    try {
      await completePasswordReset(idUtilizador, secret, password);
      setComplete(true);
    } catch {
  ...
  return (
    <main className="relative flex min-h-dvh items-center justify-center overflow-hidden bg-[var(--background)] px-4">
      ...
      <div className="relative z-10 w-full max-w-[420px]">
        ...
        <div className="mb-8 flex flex-col items-center text-center"><Logo size={48} className="mb-4 brightness-150 contrast-125" /><h1 className="text-2xl font-semibold tracking-tight text-[var(--foreground)]">Nova palavra-passe</h1><p className="mt-2 text-sm text-[var(--muted-foreground)]">Escolha uma palavra-passe forte para proteger a conta.</p></div>
        <GlassCard className="p-6 sm:p-8">
          {complete ? (
            <div className="relative z-10 text-center"><CheckCircle2 className="mx-auto h-12 w-12 text-emerald-400" /><h2 className="mt-5 text-lg font-semibold text-[var(--foreground)]">Palavra-passe atualizada</h2><p className="mt-2 text-sm text-[var(--muted-foreground)]">Já pode entrar na sua conta com a nova palavra-passe.</p><GlassButton href="/login" variant="primary" className="mt-6 w-full">Entrar</GlassButton></div>
          ) : (
            <form className="relative z-10 space-y-4" onSubmit={handleSubmit}>
              <div className="space-y-2"><label htmlFor="new-password" className="text-sm font-medium text-[var(--foreground)]">Nova palavra-passe</label><div className="relative"><Lock className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--muted-foreground)]" /><input id="new-password" type={showPassword ? "text" : "password"} autoComplete="new-password" value={password} onChange={(event) => setPassword(event.target.value)} className="glass-input w-full py-2.5 pl-10 pr-10 text-sm" disabled={loading} /><button type="button" onClick={() => setShowPassword((value) => !value)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[var(--muted-foreground)] hover:text-[var(--foreground)]" aria-label={showPassword ? "Ocultar palavra-passe" : "Mostrar palavra-passe"}>{showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button></div></div>
              <div className="space-y-1">{passwordChecks.map((check) => <p key={check.label} className={`text-xs ${check.pass ? "text-emerald-400" : "text-[var(--muted-foreground)]"}`}>{check.pass ? "✓" : "○"} {check.label}</p>)}</div>
              <div className="space-y-2"><label htmlFor="confirm-password" className="text-sm font-medium text-[var(--foreground)]">Confirmar palavra-passe</label><input id="confirm-password" type={showPassword ? "text" : "password"} autoComplete="new-password" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} className="glass-input w-full px-4 py-2.5 text-sm" disabled={loading} /></div>
...
export default function ResetPasswordPage() {
  return <Suspense fallback={<main className="flex min-h-dvh items-center justify-center bg-[var(--background)]"><Loader2 className="h-5 w-5 animate-spin text-[var(--muted-foreground)]" /></main>}><ResetPasswordForm /></Suspense>;
}
