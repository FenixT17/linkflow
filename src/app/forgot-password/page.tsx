  ...
  return (
    <main className="relative flex min-h-dvh items-center justify-center overflow-hidden bg-[#030303] px-4">
      ...
      <div className="relative z-10 w-full max-w-[420px]">
        ...
        <div className="mb-8 flex flex-col items-center text-center"><Logo size={48} className="mb-4" /><h1 className="text-2xl font-semibold tracking-tight text-white/90">Recuperar palavra-passe</h1><p className="mt-2 text-sm text-white/50">Recebe um link seguro para criar uma nova palavra-passe.</p></div>
        <GlassCard className="p-6 sm:p-8">
          {sent ? (
            <div className="relative z-10 text-center"><CheckCircle2 className="mx-auto h-12 w-12 text-emerald-400" /><h2 className="mt-5 text-lg font-semibold text-white/90">Verifica o teu email</h2><p className="mt-2 text-sm leading-relaxed text-white/55">Se existir uma conta com esse email, receberás instruções para recuperar a palavra-passe.</p><GlassButton href="/login" variant="secondary" className="mt-6 w-full">Voltar ao login</GlassButton></div>
          ) : (
            <form className="relative z-10 space-y-4" onSubmit={handleSubmit}>
              ...
              <div className="relative"><Mail className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-white/40" /><input id="reset-email" type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} className="glass-input w-full py-2.5 pl-10 pr-4 text-sm" disabled={loading} required /></div>
